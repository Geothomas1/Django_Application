...
class HttpInterceptor {
  ...
  String _baseUri = 'http://192.168.1.5:8000';
  ...
  Future<HttpResponse?> restCall(
    ...
    try {
      print(
          '$request  http:/$url request_body=> $body queryParams=> $queryParams');
      switch (request) {
        case Request.GET:
      ...
      switch (response.statusCode) {
        ...
        case 500:
          print(
              'Internal Server Error, please retry later ${response.statusCode}');
          // Utils.showToast(msg: 'Internal Server Error, please retry later');
          return null;
        default:
          print(
              'Something Went Wrong, please retry later ${response.statusCode}');
